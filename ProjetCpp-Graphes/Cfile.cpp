...
char ** Cfile::FILparse_file(const char* pcFile_to_parse) throw (Cexception)
{
	...
	if (streamOfFileToParse.is_open() == true)
	{
		...
		while (streamOfFileToParse.getline(pcBuffer, BUFFER_SIZE))
		{
			...
			pcValue = strtok_r(pcBuffer, CHARS_DELIMS_TOKENS, &pcNext_value); // Trying to retrieve a token into pcValue
			...
			while (pcValue != NULL)
			{
				...
				pcValue = strtok_r(NULL, CHARS_DELIMS_TOKENS, &pcNext_value);
			}
		}
